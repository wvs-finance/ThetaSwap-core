...
function TimeSeriesChart({
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full" preserveAspectRatio="none" style={{ height: '100%' }}>
      ...
      <line x1={pad.left + 60} y1={6} x2={pad.left + 72} y2={6} stroke="#64748b" strokeWidth={1} strokeDasharray="3,2" />
      <text x={pad.left + 76} y={9} className="fill-zinc-500" fontSize={7} fontFamily="'IBM Plex Sans', sans-serif">
        HWM
      </text>
    </svg>
  )
            ...
            <table className="w-full min-w-[580px] border-collapse">
              <thead className="sticky top-0 z-10 border-b border-zinc-800 bg-zinc-900/95 backdrop-blur-sm">
                <tr>
                  ...
                  />
                  <PosColHeader
                    label="Max Delta-plus"
                    col="maxDelta"
                    align="right"
                    tooltip="Highest Delta-plus epoch observed during this position's lifetime"
                  />
                </tr>
              </thead>
              <tbody>
        ...
        <div className="flex flex-1 flex-col overflow-auto">
          ...
          <div className="border-b border-zinc-800 p-3">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="grid flex-1 grid-cols-2 gap-x-4 gap-y-1.5">
                <MetricRow label="Delta-plus Cumul." value={oracle.deltaPlusCumulative.toFixed(3)} color={getSeverityColor(oracle.deltaPlusCumulative)} />
                <MetricRow label="atNull" value={oracle.atNull.toFixed(3)} tooltip="Competitive threshold: Delta-plus = max(0, A_T - atNull)" />
                <MetricRow label="theta-sum" value={oracle.thetaSum.toFixed(2)} tooltip="sum(1/block_lifetime_k) cumulative over position removals" />
          ...
          <div className="flex-1 p-3">
            ...
            <p className="mb-2 text-[10px] text-zinc-500">
              LONG payout vs Delta-plus -- power-squared: max(0, ((HWM - K)/(1 - K))^2)
            </p>
            <PayoffCurve
              data={payoffCurve}
