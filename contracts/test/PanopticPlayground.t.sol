...
contract PanopticPlaygroundTest is AngstromTest, PanopticPoolTest {
    ...
    function swapToStrike(
        ...
        for (uint256 i = 0; i < 10; ++i) {
            ...
            actor.swap(
                _poolKey, false, -type(int256).max,
        ...
        delta = actor.swap(
            ...
            false,
            -type(int256).max,
            TickMath.getSqrtPriceAtTick(strike)
        );
    }
	...
	console2.log("=====================PRE-CONDITIONS(WRITING THE OPTION )================================");
	initPool();
	...
	actor.modifyLiquidity(
	    ...
	    int24(0),
	    int24(360),
	    int256(uint256(DEFAULT_START_LIQUIDITY)),
	    bytes32(0)
	);

	console2.log("============================TEST(WRITING THE OPTION)============================");
	...
	console2.log("==========================POST-CONDITIONS (WRITING THE OPTION )==========================================");

	console2.log("Seller C :: Option Notional: ", sfpm.balanceOf(address(pp), TokenId.unwrap(tokenId)));
	console2.log("Since the option is a put, it MUST be cash-secured. The CollateralTracker tracks token B because A is measured against B (token 0 = A, token 1 = B).", "");
	...
	(uint128 balance, uint64 poolUtilization0, uint64 poolUtilization1) = ph.optionPositionInfo(pp, Seller, tokenId);

	console2.log("Seller C :: Option Nominal Balance :", balance);
	console2.log("Collateral tracking of B is now utilized by Seller C. Utilization = Seller C's marginDeposited / totalSupply of the collateral tracker (the deflator/normalizer)", Math.mulDivRoundingUp(amount1, 10000, ct1.totalSupply()));
	...
	console2.log("=================PRE-CONDITIONS (REALIZING OPTION PAYOFF) ==========================", "");

	{
	    TokenId[] memory posIds = new TokenId[](1);
	    posIds[0] = tokenId;
	    (LeftRightUnsigned shortPremiaAccrued, , ) = pp.getAccumulatedFeesAndPositionsData(
	        Seller,
	        true,
	        posIds
	    );
	    console2.log("Seller C :: Accrued streaming premia token0: ", shortPremiaAccrued.rightSlot());
	    console2.log("Seller C :: Accrued streaming premia token1: ", shortPremiaAccrued.leftSlot());
	}
	
	console2.log("The only way to realize the payoff is by having the price move. This is achieved only through trading volume that drives prices. Given the volatility and expiration proxies the option writer used, we apply the same argument to derive the trading volume needed to drive the price to the option writer's desired ITM level and realize the intended payoff", "");


	

	console2.log("===================TEST (REALIZING OPTION PAYOFF)==================================", "");
	console2.log("Alice, who acts as a trader, will realize the payoff through her swap order, which effectively puts Seller in the money:","");
	vm.prank(Alice);
	angstrom.execute("");
	...
	console2.log("==================POST - CONDITIONS (REALIZING OPTION PAYOFF)=======================","");
	...
	console2.log("==========================PREVIEW (PREMIA & PAYOFF)==========================================", "");
	...
	{
	    (uint128 balanceITM, uint64 poolUtil0ITM, uint64 poolUtil1ITM) = ph.optionPositionInfo(pp, Seller, tokenId);
	...
	console2.log("==========================BURN (REMOVING POSITION)==========================================", "");
	...
	console2.log("==========================POST-BURN ASSERTIONS==========================================", "");

	assertEq(sfpm.balanceOf(address(pp), TokenId.unwrap(tokenId)), 0, "Position not fully burned");
...

	{
	    ...
	    console2.log("Seller C :: P&L from burn only (token1): ", int256(_balAfter) - int256(_balBefore));
	    console2.log("Seller C :: Total lifecycle P&L (token1): ", int256(_balAfter) - int256(_balStart));
	}
