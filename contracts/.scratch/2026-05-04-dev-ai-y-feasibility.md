...
> **Anti-conflation note (per CORRECTIONS-ι 2026-05-04 at `path-b-stage-2/phase-1/corrections_iota_a_s_framing.md`)**: Y_s3 (UNCTADstat EBOPS-9 cross-LATAM panel) is a **Y sensitivity arm at the cross-LATAM aggregate level**, NOT an a_s instrument-side variable. Per DRAFT.md simplest model `a_s = Υ - Σ q_t/(X/Y)`, a_s is a per-user instrument-level balance sheet — calibrated at Stage-2 M-sketch from AI-vendor public pricing + DevSurvey LATAM cohort distributions, NOT measured as a country-aggregate macro variable. BoP "computer services imports" framing as a_s would require nesting 4 lossy fractions (ICT → SaaS → AI → developer-paid LATAM-resident); CORRECTIONS-ι rejects that framing and pins per-user a_s instead. This memo's §2.1-§2.5 BoP discussion is correctly scoped to Y_s3 sensitivity, NOT a_s.
