...
> **Status:** Rev 4.1 (2026-04-24). See Revision history below. **Rev 4.1 extends Rev 4 Phase 1.5.5 along two dimensions: (1) Task 11.F scope expanded from peak-day event research to include user-intent classification (what are cCOP/COPM txns actually for? — UBI, retail payments, arbitrage, treasury, remittance), data-source correctness verification (does Dune #7366593 capture the true flow, or are we missing MiniPay, Carbon DeFi, Uniswap-V3-cCOP/cUSD, CeFi exchange paths?), and transaction-size distribution vs remittance-size benchmark (~$350/tx Colombian median per World Bank KNOMAD); (2) a new **Task 11.K (EXIT plan / kill criterion)** formally defines the conditions under which we halt rather than force a non-existent remittance relationship. Kill triggers include: intent-research shows >70% non-remittance volume, data-source is systematically wrong, transaction-size distribution is payment-sized not remittance-sized, or Task 11.H argmax M_max < 0.40 across the full pre-committed F. If any trigger fires, Phase-A.0 EXITs to a pivot candidate (explicit option: re-scope from "remittance surprise" to "on-chain payment/consumption surprise" with BanRep retail-sales or consumption index as validation anchor). Non-stop policy coexists with exit criterion: "iterate until argmax M ≥ M_threshold OR kill criterion fires." Rev-1.1.1 at `ac5189363` remains SUPERSEDED; no Rev-2 spec authoring and no Phase 2b execution until Phase 1.5.5 converges or EXITs.**
