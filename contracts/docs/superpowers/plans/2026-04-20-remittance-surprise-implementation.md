...
- [ ] **Step 1 (Rev-2 scoped — CR B2): Write the failing test, restricted to primary-RHS only.** Assert `load_cleaned_remittance_panel(conn) → CleanedRemittancePanelV1` exists, where `CleanedRemittancePanelV1` is a new frozen dataclass mirroring `CleanedPanel` but adding **only** the remittance primary-RHS column (no auxiliary columns, no quarterly-corridor column — those extend the dataclass in Tasks 13 and 14). Assert it inherits the Rev-4 `LockedDecisions` + extends with remittance-primary-RHS decision identifiers only. Assert the existence (but not the full behavior) of a `_compute_decision_hash_remittance` seam that the Task-12 test will exercise end-to-end; Task 9 only asserts the primary-RHS column is hashed. Full-dataclass validation (V1 → V2 aux columns → V3 corridor) is deferred to Task 15 panel-integration.
- [ ] **Step 2: Run and confirm failure.**
...
- [ ] **Step 1: Write the failing test.** Assert `construct_ar1_surprise(series, pre_sample_end_date, vintage_policy) → SurpriseSeries` exists. Assert the pre-sample / rolling refit policy pinned in the Rev-1 spec is honored. Assert the step-interpolation side (LOCF vs next-fill, per Rev-1 spec) is honored exactly.
- [ ] **Step 2: Run and confirm failure.**
...
- [ ] **Step 1: Write the failing test.** Assert four auxiliary columns exist on the loaded panel: `regime_post_2015` (binary dummy), `event_petro_trump_2025` (binary dummy for documented Jan-2025 48h window), **`a1r_monthly_rebase`** (the primary remittance surprise re-aggregated to monthly cadence — named `a1r_` to disambiguate from the CPI `a1_monthly_rebase` per RC F2 and CPI-FAIL pre-registered pivot-candidate `A1-C`), `release_day_indicator` (binary). Each column's computation rule matches the Rev-1 spec exactly. Assert each aux-column hash extends the decision-hash produced by Task 12's seam (i.e., the extended-hash after Task 13 is deterministic, includes all four aux-column spec hashes in sorted order, and still preserves the Rev-4 base hash). This test absorbs the aux-column portion of CR B1.
- [ ] **Step 2: Run and confirm failure.**
- [ ] **Step 3: Implement** extending `CleanedRemittancePanelV1` → `CleanedRemittancePanelV2` additively.
- [ ] **Step 4: Run and confirm pass.**
- [ ] **Step 5: Commit** with message `feat(remittance): auxiliary columns (regime/event/A1-R/release-day) per Rev-1 spec; hash extension`.
...
- Create: `contracts/notebooks/fx_vol_remittance_surprise/Colombia/estimates/nb2_full.pkl`
...
- [ ] **Step 1: Write the failing test.** Assert §7 (all primary + sensitivity rows: primary, alternate-LHS, **A1-R monthly** (renamed from A1 monthly per RC F2 to disambiguate from CPI A1-C), release-day-only, pre/post-2015 Quandt-Andrews-windowed, Petro-Trump event-dummied, quarterly corridor reconstruction — or the documented-gap placeholder if Task 14 Step 0 decided no-recipe; plus any Rev-1-spec additions). Assert `nb3_forest.json` is emitted with one row per sensitivity (β̂, SE, CI bounds, row label). Assert `nb3_sensitivity_table.json` is emitted with the same rows plus gate-pass/fail flags per row. Assert §8 (forest-plot render at `figures/forest_plot.png`; rows sorted per Rev-1 spec). Assert §9 (anti-fishing halt condition — material-mover §9 spotlight is emitted ONLY if primary T3b PASSes; otherwise §9 cells emit empty placeholders referencing the design doc's anti-fishing framing and the §8 sensitivity-row pre-registration; A1-R monthly + release-day-excluded rows are preserved as pre-registered sensitivities, not post-hoc rescue claims).
...
- [ ] **Step 1:** Dispatch Technical Writer to author the completion memory containing: verdict digest (PASS or FAIL), β̂_remittance + SE + CI, all test verdicts (T1-T7), reconciliation outcome, all sensitivity-row outcomes (primary, alternate-LHS, A1-R monthly, release-day-only, pre/post-2015 Quandt-Andrews, Petro-Trump event-dummied, quarterly corridor reconstruction or its documented-gap placeholder), anti-fishing framing re-asserted (Phase-A.0 is a distinct pre-commitment on the remittance external-inflow channel, not a rescue of the CPI-FAIL), pointers to all 11 artifacts (`nb1_panel_fingerprint.json`, `nb2_params_point.json`, `nb2_reconcile.json`, `nb2_full.pkl`, `nb3_forest.json`, `nb3_sensitivity_table.json`, `gate_verdict_remittance.json`, `README.md`, `forest_plot.png`, plus MPR-source provenance and corridor-reconstruction-recipe scratch notes), lessons learned, Phase-A.1 go/no-go recommendation conditional on verdict.
...
- MDES → Task 1 Step 4 + Task 22b §4 (numerical MDES assertion in T3b replay) + Task 27 (Model QA audit)
...
- Andrews bandwidth → Task 19 §3
- Interpolation side → Task 10 + Task 12 (hash-extension seam ties to interpolation choice)
- AR order → Task 10
- Vintage discipline → Task 11 (MPR-vintage-date column; manual re-pull per Rev-2 data-access correction)
- Reconciliation rule under heteroskedasticity → Task 21b §8 (directional or numerical-intersection per Rev-1 spec row 9)
...
All 13 covered across Phases 0/2/3/4.
...
Anti-fishing framing appears in Task 8 (notebook headers), Tasks 22b-23 (NB3 §9 halt condition), Task 30a (completion memory). Three-way reviews at Phase 0 (Tasks 2-4 spec review CR+RC+TW per `feedback_three_way_review.md`), Phase 3 intra-phase gates (Tasks 18a, 21d, 24c CR+RC+SD per `feedback_implementation_review_agents.md`, Rev-2 inserts), Phase 4 (Tasks 27-28 impl review).
