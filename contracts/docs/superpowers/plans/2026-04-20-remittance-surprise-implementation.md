...
**Gate:** all 4 CSVs + provenance README + profile `.scratch` committed.
...
- [ ] **Step 2:** declare table schemas in `econ_schema.py` (varbinary for addresses, timestamp for dates, uint256 for amounts, matching Dune decoded-table types from Task 11.M).
...
**Inputs:** Task 11.L literature review (mandatory guardrail); Task 11.M profile; Task 11.N X_d; Rev-4 panel (TRM RV, CPI surprise, Banrep repo rate, Fed funds) loaded via `econ_query_api.py`.
