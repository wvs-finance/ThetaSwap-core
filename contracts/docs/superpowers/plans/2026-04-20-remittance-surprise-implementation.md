...
- Python `web3.py` OR direct `requests` JSON-RPC
...
- Modify: `contracts/scripts/econ_schema.py` — ADD a second `onchain_copm_transfers` table for the full dataset (keep `onchain_copm_transfers_sample` as historical-sample pointer) OR replace it in-place with a `is_sample` flag column defaulted to FALSE after this task (pick whichever is cleaner at implementation time; document choice in commit message)
- Modify: `contracts/scripts/econ_query_api.py` — expose `load_onchain_copm_transfers()` returning frozen-dataclass for the FULL dataset; update existing `load_onchain_copm_transfers_sample()` semantics (deprecate or re-route to the same full loader with sample=True filter)
- Modify: `contracts/scripts/tests/test_onchain_duckdb_migration.py` — update expected row count; add assertion that full transfers table round-trips byte-exactly via the same checksum pattern
- Modify: `contracts/scripts/copm_xd_filter.py` — no code changes required; when re-run on full data, `compute_weekly_xd()` now produces distribution-channel X_d naturally (the filter was written to operate on full transfers; only the input data changes)
- Modify: Task 11.N's `onchain_xd_weekly` table — add/update rows with `proxy_kind = "b2b_to_b2c_net_flow_usd"` alongside the existing supply-channel series; Task 11.O consumes both for cross-validation
...
**Inputs:** Task 11.L literature review (mandatory guardrail); Task 11.M profile; Task 11.N X_d; Rev-4 panel (TRM RV, CPI surprise) **plus Task 11.M.6 panel extension (Fed funds + Banrep IBR rate-level columns)** loaded via `econ_query_api.py`.
...
- [ ] **Step 2:** derive all 13 rows of the resolution matrix consuming Task 11.L recommendations (expected sign, magnitude, identification strategy). Formulate the functional equation + null hypothesis pre-committed before estimation.
- [ ] **Step 3 (MDES — fixes Rev-1.1.1 CR-E2):** compute MDES via `scipy.stats.ncf.ppf` root-finding at actual N_eff and df₂. Do NOT use the analytical approximation that produced Rev-1.1.1's λ ≈ 13 overstatement. Verify with an independent-reproduction witness (second compute path via `statsmodels.stats.power.FTestPower` or manual non-central-F root-find).
...
**Gate:** Rev-2 spec committed with functional equation + pre-committed null + scipy-correct MDES + literature-grounded identification assumptions. All 13 resolution-matrix rows populated.
