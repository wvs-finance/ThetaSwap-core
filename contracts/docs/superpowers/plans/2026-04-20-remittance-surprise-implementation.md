...
> **Status:** Rev 4 (2026-04-24). See Revision history below. **Rev 4 is a structural escalation triggered by (a) unanimous "needs fixes" at Task 11.E Rev-1.1.1 three-way review (CR REJECT + RC NEEDS WORK + TW NEEDS FIXES, including independently-verified MDES arithmetic error), and (b) a user-surfaced cCOP representativeness concern documented in prior research at `/home/jmsbpp/apps/liq-soldk-dev/notes/structural-econometrics/identification/2026-04-02-ccop-qa-audit.md:73` — the raw Dune #7366593 aggregate is dominated by a small power-user group (Medellín-early-adopters + bots/treasury), not representative of household remittance behavior. Rev 4 inserts Phase 1.5.5 (Tasks 11.F–11.J) under a NON-STOP filter-design policy: iterate filter families against a pre-committed composite symmetric similarity objective M(filtered_on_chain, BanRep_quarterly) until argmax; Rule 13's 3-cycle cap does NOT apply to filter iteration (it applies only to the Rev-2 spec three-way review at Task 11.J). Rev-1.1.1 at `ac5189363` is SUPERSEDED. No Rev-2 spec authoring and no Phase 2b execution until Phase 1.5.5 converges.**
...
**Non-stop filter-design policy.** Tasks 11.G and 11.H form an iterative search loop with a pre-committed objective: find the filter F that maximizes a composite symmetric similarity measure M(filter_applied_to_on_chain_series, BanRep_quarterly_remittance). **Rule 13's 3-cycle cap does NOT apply to filter iteration within Task 11.H** — the cap applies only to Rev-2 spec three-way review at Task 11.J. The "non-stop" guardrail is pre-commitment, not cycle-limiting: M, F, and the weights across component similarity measures are pre-committed in Task 11.H Step 1 BEFORE any filter is evaluated, and mid-search modification is banned.

### Task 11.F: Peak-day event research (Rev 4 insert)

**Subagent:** background-mode general-purpose subagent with WebSearch + WebFetch (and Dune MCP for any on-chain disambiguation).

**Files:**
- Create: `contracts/.scratch/2026-04-24-ccop-peak-day-event-research.md`
- Input: `contracts/data/copm_ccop_daily_flow.csv`

- [ ] **Step 1:** Enumerate the top 30 single-day flow events ranked by `abs(ccop_usdt_inflow_usd) + abs(ccop_usdt_outflow_usd) + abs(copm_mint_usd) + abs(copm_burn_usd)`. Record date, magnitude, and roundtrip ratio `|inflow − outflow| / max(inflow, outflow)` (ratio < 0.01 is arbitrage-suspect).
- [ ] **Step 2:** For each top-30 date, run web-search queries combining: `"cCOP" OR "Celo" OR "Mento" OR "COPM" OR "Minteo" <date>`; `"Colombia stablecoin" <date>`; `"Celo Colombia DAO" <date>`; `"Mento broker" migration <date>`; `"airdrop" Celo <month-year>`. Prioritize forum.celo.org, Mento blog, Celo blog, Medellín tech community posts.
- [ ] **Step 3:** Classify each peak by event-type: (a) migration/technical upgrade, (b) airdrop/liquidity-mining campaign, (c) governance-call / DAO disbursement, (d) arbitrage opportunity (TRM dislocation / exchange depeg), (e) treasury operation (single-entity mint/burn), (f) unknown.
- [ ] **Step 4:** Propose ≥3 candidate dummy variables / instruments with proposed windows (e.g., `D_migration_2026_01_24_to_26`, `D_airdrop_week_YYYY_WW`, `D_governance_thursday`, `D_tumbler_event_<date>`). Each proposal includes (i) event justification, (ii) window justification, (iii) whether the dummy is exogenous to TRM-RV.
- [ ] **Step 5:** Report findings in `.scratch` with per-date evidence citation (URL + excerpt ≤15 words per copyright hygiene). Output consumed by Task 11.G (filter brainstorm) and Task 11.H (pre-committed search space). No code written in Task 11.F.

**Recovery protocol:** if web-search returns nothing for >50% of top-30 dates, escalate to user before Task 11.G (may need manual Twitter/X archive review or on-chain address-labeling research instead).

**Gate:** Task 11.F completes when the `.scratch` report is authored and committed. No separate review; findings are first-class inputs to Task 11.G/H which apply their own review gates.

### Task 11.G: Power-user filter brainstorm (Rev 4 insert)
...
### Task 11.H: Non-stop filter iteration under pre-committed symmetric similarity objective (Rev 4 insert)
...
### Task 11.I: Rev-2 spec authoring via `structural-econometrics` skill re-invocation (Rev 4 insert)
...
### Task 11.J: Rev-2 spec three-way review (Rev 4 insert)
