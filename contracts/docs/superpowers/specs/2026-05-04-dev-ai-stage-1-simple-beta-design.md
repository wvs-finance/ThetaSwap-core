...
decision_hash_v1_0_2: 7c72292516f58f3cf2d16464d4f84c3e7d7270ad2c5d3d8ed8fef6b3b2751f5a
...
prior_decision_hash_v1_0_1: 456ba39e188d00bb17471359a5803d6aa8a40de3b3788f17294bab828a968204  # superseded 2026-05-05 by v1.0.2 CORRECTIONS-κ inline integration (Option A from Task 1.1 FLAG-A + FLAG-B disposition memo)
...
verifier_v1_0_1_wave1: pending (RC closure-only on diff, post-v1.0.1 emission; superseded by v1.0.2)
verifier_v1_0_2_wave1: pending (RC + purpose-matched specialist closure-only on diff, post-v1.0.2 emission per `feedback_two_wave_doc_verification`)
revision_history:
  ...
  - v1.0.1 2026-05-05 hybrid-FLAG integration (Option D from disposition memo): inline integration of MQS FLAG-1 (Section J ⊂ Pair D G–T compositional-accounting acknowledgment as §1 paragraph + new §9.16 invariant; R5 robustness arm DEFERRED to v1.1 conditional gate per disposition memo); MQS FLAG-2 (Section-J-specific logit-derivative amplification quantified in §5.1, ratio 2.34× vs Pair D 1.32×); MQS FLAG-3 (§6 fallback (i) trigger restructured to require §9.5 HALT unconditionally, replaces unpinned "5× Pair D residual variance" reference); MQS FLAG-5 (§4 explicit X-back-extension justification); MQS NIT-1 (§5.1 Y_s1 redesign-dummy pinned as option (a) per Y feasibility memo §1.2); MQS NIT-2 (§5.1 Section J 4-digit code range replaced with Division-list formulation). MQS FLAG-4 (Y_s3 cluster-SE / N_MIN unit) DEFERRED to v1.1 conditional on §8.3 (i) Y_s3 promotion. RC NIT-1/2/3 + MQS NIT-3 deferred (cosmetic). sha256 `456ba39e188d00bb17471359a5803d6aa8a40de3b3788f17294bab828a968204`
  - v1.0.2 2026-05-05 CORRECTIONS-κ inline integration + Phase-4→Phase-3 typo fix: post-Task-1.1 PASS-WITH-FLAG disclosure (user pick A from Task 1.1 disposition memo at `contracts/.scratch/dev-ai-stage-1/disposition-memo-task-1.1-flag-A-flag-B.md`); §1 new CORRECTIONS-κ acknowledgment paragraph; §5.1 quantified realized-vs-anticipated gap (cell_count [94, 267] vs memo [700, 1200]; raw_share [0.014, 0.031] vs spec [0.04, 0.10]; logit-derivative amplification 33-73 i.e. 3-7× larger than v1.0.1-anticipated 2.34× baseline); §6 R1 + R3 escalation discipline tightened under realized κ-amplification (primary-vs-{R1, R3} sign-inconsistency MANDATORY ESCALATE per §3.3 Clause-A; either-disagreeing fires, no judgment-call escape); new §9.17 invariant binding Phase-3 result-memo new §11.X disclosure section pre-pin (extends §9.16); Phase-4 → Phase-3 typo corrected at 5 occurrences (lines 40, 191, 224, 258, 374 in v1.0.1; phase-numbering aligned to plan v1.1 which has Phase 3 = memo + 3-way review and Phase 4 = disposition per RC FLAG-2 closure 2026-05-05). decision_hash_v1_0_2 sha256 `7c72292516f58f3cf2d16464d4f84c3e7d7270ad2c5d3d8ed8fef6b3b2751f5a`
---
...
**Compositional-accounting risk acknowledgment (per Wave-1 RC FLAG #1 + Wave-2 MQS FLAG-1 v1.0 closure inherited as v1.0.1 §1 paragraph + load-bearing in §9.16).** Section J is a strict subset of Pair D's Section G–T, and Pair D's broad-services aggregate already PASSed Stage-1 with β_composite = +0.137. A positive β_composite on Section J is therefore consistent with EITHER (i) the dev-AI-cost transmission mechanism firing independently at the ICT-narrow subsector level, OR (ii) Section J's compositional contribution to Pair D's broad-services PASS — i.e., a re-discovery of the Pair D signal aggregated up to ICT. **The Stage-1 verdict here cannot distinguish these two possibilities without a Section-J-vs-Section-(G–T-minus-J) decomposition.** This is acknowledged here in §1 as a verdict-interpretation caveat; the Stage-1 PASS routing language in §8.3 is bound by §9.16 to flag this ambiguity in the Phase-3 result memo, requiring downstream Stage-2 M-sketch authoring to address the decomposition explicitly. Adding an R5 robustness arm "primary spec on (Sections G–T minus J)" — where a similar β_composite would imply Section J is compositional, while a smaller β_composite would imply Section J's narrowing has an independent ICT signal — was DEFERRED at v1.0.1 authoring time per the disposition memo Option D (not added at this revision; conditional gate for v1.1 if user explicitly invokes the design change OR if Stage-2 dispatch surfaces the need). The compositional-accounting risk inherits the Pair D RC FLAG #1 "hedge the *correlation* not the *BPO causal channel*" caveat and binds *more strongly* here because the Y is a strict subset of the already-PASSing Y rather than an independent panel.

**CORRECTIONS-κ post-Task-1.1 ex-ante-expectation gap acknowledgment (NEW v1.0.2; per Task 1.1 PASS-WITH-FLAG disposition memo user pick A).** Phase-1 Task 1.1 (DANE GEIH Section J + Section M micro-data ingest) completed schema-stable (134/134 months produced canonical Rev.4 codes; `DevAIStage1RAMA4DRev4ContradictionPathological` did NOT fire; free-tier compliance 100%; N=134 ≥ N_MIN=75 by wide margin) but surfaced two diagnostic FLAGs on Section J's empirical character: **FLAG-A** Section J `cell_count` realized at `[94, 267]` (median 145) vs Y feasibility memo §1.1 ex-ante baseline `[700, 1200]` — a factor 5-7× below memo expectation, with 1 month at `cell_count = 94` (2024-10-31, borderline rare-event regime under 100) and 74 of 134 months (55%) below 150; root cause is that the Y feasibility memo §1.1 estimated Section J ≈ 10-15% of broad services, whereas Section J is empirically ~3-4% of broad services. **FLAG-B** Section J `raw_share` realized at `[0.014, 0.031]` vs spec §5.1 v1.0.1 expected range `[0.04, 0.10]` — a factor 1.3-3× lower; consequently the logit derivative `d/dY[logit(Y)] = 1/[Y(1-Y)]` at the realized range maps to `[33, 73]`, a factor 3-7× larger amplification than the v1.0.1-anticipated 2.34× baseline (the v1.0.1 baseline was estimated against the `[0.04, 0.10]` anticipated range; the realized range produces materially higher curvature). Logit-OLS validity is preserved (all values well-interior to `(0, 1)`; logit_share finite range −4.24 to −3.43); the FLAGs are diagnostic-not-blocking, and Section M (Y_s2) is comparatively healthy (`cell_count [136, 245]`, CoV 0.119 vs Section J CoV 0.252) but the primary spec is unchanged per disposition memo Option A. Spec §5.1 v1.0.1 lines 186-191 PRE-PINNED two hedges (R1 = 2021 regime dummy on logit-Y per §6; R3 = raw-OLS no logit per §7) specifically for logit-amplification scenarios; the realized amplification IS the scenario the spec anticipated, just at higher intensity than the v1.0.1 baseline. Under v1.0.2 CORRECTIONS-κ discipline, R1 + R3 are MORE load-bearing than under v1.0.1: primary-vs-{R1, R3} sign-inconsistency triggers Clause-A ESCALATE per §3.3 (already pre-pinned in v1.0.1); v1.0.2 κ-tightens this condition to require BOTH R1 AND R3 to AGREE-OR-ESCALATE rather than either-or judgment-call (see §6 v1.0.2 escalation tightening sub-paragraph). Cross-references: §5.1 (where realized-vs-anticipated gap is quantified with concrete numerics); §6 (where R1 + R3 escalation discipline is tightened under realized κ-amplification); §9.17 NEW (where Phase-3 result-memo new §11.X disclosure section is pre-pinned, extending §9.16 compositional-accounting verdict-interpretation discipline). **Anti-fishing posture (explicit).** The FLAGs were surfaced BEFORE β estimation; the pre-registered hedges (R1, R3) were specifically designed for logit-amplification scenarios per spec §5.1 v1.0.1; CORRECTIONS-κ records the realized-vs-anticipated gap in the spec rather than silently adjusting thresholds, post-hoc smoothing the data, or post-hoc widening the outcome variable. This is the discipline-respecting path under `feedback_pathological_halt_anti_fishing_checkpoint` (HALT + disposition memo + ≥3 pivot options + user adjudication; never silent threshold tuning); the alternative pivot options (B Y_s2 promote / C 3-month rolling smoother / D widen to {J ∪ M} / E re-architect) are all enumerated in the disposition memo and were considered, but Option A was selected precisely because it is the anti-fishing-cleanest of the five. The 94-cell rare-month observation is post-2021 era and therefore captured by R1's regime-dummy interaction term; R3's raw-OLS sidesteps the logit nonlinearity entirely and is the cleanest κ-amplification cross-check.

**Section J literature-anchor sub-aggregate-substitutability flagged (per Wave-1 RC FLAG #1).** The transmission-chain inheritance from "BPO offshoring works in Philippines" → "Section J offshoring works in Colombia" rests on a sub-aggregate-substitutability assumption: Section J Divisions 58–63 contain BPO-relevant Divisions 62 (Computer programming) + 63 (Information service activities), but ALSO contain non-BPO Divisions 58 (Publishing), 59 (Motion picture/video), 60 (Programming & broadcasting), and 61 (Telecommunications) — sectors that respond to OTHER demand drivers (telecom regulatory cycles, advertising spend, content market) than US tech-firm offshoring. If Section J β_composite > 0 turns out positive, attribution to "dev-AI offshoring channel" cannot be made cleanly without additional sub-Division decomposition. This is also bound by §9.16 as a Phase-3 result-memo disclosure requirement; Stage-2 M-sketch dispatch (if Stage-1 PASSes) MUST calibrate hedge geometry against the ICT-services-direct sub-component (Divisions 62–63), NOT against the Section J aggregate β_composite.
...
The **logit nonlinearity at Section J's share range is ~2.3× more curved** than at Pair D's. Operationally: a small raw-share level error (e.g., DANE empalme residual after Marco-2018 frame change, or sectoral-classification-boundary noise in `RAMA4D_R4`) maps to a **2-6× larger logit-Y level error** at Section J's range than at Pair D's. The §6 empalme-residual-bias acknowledgment (`d/dY[logit(Y)] = 1/[Y(1-Y)]` varying across the support) is therefore *more* load-bearing here than in Pair D, and the design's hedges against this amplification are: (a) the §6 R1 (2021 regime dummy on logit-Y) which probes whether the empalme residual interacts with the methodology-break disposition; (b) the §7 R3 (raw-OLS, no logit transform) which sidesteps the nonlinearity entirely; (c) the §6 cell-size HALT-only fallbacks (§9.5 protocol) which catch sample-variance-driven Y volatility before it propagates through the logit derivative. Cross-reference: §6 line 244 (empalme residual-bias) AND §7 R3 (raw-OLS robustness) bind to this amplification concern. Promoting a primary-PASS verdict without acknowledging the Section-J-specific amplification in the Phase-3 result memo when R1 / R3 are inconsistent with the primary is anti-fishing-banned per §9.

**Realized data update (CORRECTIONS-κ v1.0.2 inline; post-Task-1.1).** Section J `raw_share` is realized at `[0.014, 0.031]` (134 months 2015-01 → 2026-02; Task 1.1 DE-reported FLAG-B) — a factor 1.3-3× *below* the v1.0.1-anticipated `[0.04, 0.10]` range. Logit-derivative `d/dY[logit(Y)] = 1/[Y(1-Y)]` at the realized range maps to `[33, 73]`: at the lower bound `1/(0.014 × 0.986) ≈ 72.5`; at the upper bound `1/(0.031 × 0.969) ≈ 33.3`. This is a factor 3-7× larger amplification than the v1.0.1 baseline 2.34× across-support ratio (which was estimated against the v1.0.1-anticipated `[0.04, 0.10]` range producing across-support ratio 11.11 → 26.04 = 2.34×). The realized `cell_count` `[94, 267]` (median 145; Task 1.1 DE-reported FLAG-A) is 5-7× below the Y feasibility memo §1.1 baseline `[700, 1200]`, meaning a single sub-200-cell month maps to a wider logit-Y observation through the amplified derivative — a 10-cell sampling-noise variance at `cell_count = 100` produces a logit-Y observation variance approximately 3-7× larger than what spec §6 R1 (2021 regime dummy) and §7 R3 (raw-OLS) hedges were originally calibrated against under the v1.0.1 2.34× baseline.

**Operational discipline under realized κ-amplification (v1.0.2).** Under CORRECTIONS-κ v1.0.2 discipline, both R1 AND R3 must AGREE with primary on the sign of `β_composite` to clear the §3.3 Clause-A ESCALATE threshold. Either disagreeing fires ESCALATE per §3.3 Clause-A (κ-tightened condition; see §6 v1.0.2 escalation tightening sub-paragraph for the formal restatement). The κ-tightening is a v1.0.2-only hardening of the v1.0.1 "primary-vs-{R1, R3} sign-inconsistency triggers ESCALATE" general protocol — under v1.0.1 this was an ESCALATE candidate at orchestrator/specialist judgment; under v1.0.2 the realized 3-7× amplification factor makes R1 + R3 reliability load-bearing enough that the condition is restated as MANDATORY ESCALATE without judgment-call escape. The 94-cell rare-month observation (2024-10-31) is post-2021 era and therefore captured by R1's regime-dummy interaction term specifically; R3 (raw-OLS) sidesteps the logit derivative entirely and is the cleanest κ-amplification cross-check.
...
- **§9.16 Compositional-accounting verdict-interpretation discipline (NEW per MQS FLAG-1 HIGH + RC FLAG-1 v1.0 closure).** Section J ⊂ Pair D Section G–T is a strict-subset relationship (per §1 compositional-accounting risk acknowledgment). A positive `β_composite` on Y_p (Section J) at Stage-1 PASS / ESCALATE-PASS is consistent with EITHER (i) the dev-AI-cost transmission mechanism firing independently at the ICT-narrow subsector level, OR (ii) Section J's compositional contribution to Pair D's broad-services PASS — i.e., a re-discovery of the Pair D signal aggregated up to ICT. **The Stage-1 verdict here cannot distinguish these two possibilities without a Section-J-vs-Section-(G–T-minus-J) decomposition.** Any Phase-3 result memo emitted from this iteration MUST include (a) explicit acknowledgment of the compositional-accounting ambiguity; (b) a `β_composite_section_J` vs `β_composite_pair_D_inherited` comparison reading from the `project_pair_d_phase2_pass` memory verbatim; (c) a flagged-not-resolved status on which of (i) or (ii) is operative. Stage-2 M-sketch dispatch (if invoked downstream of a Stage-1 PASS / ESCALATE-PASS verdict) MUST address the decomposition explicitly — either by adding the deferred R5 robustness arm (primary spec on Sections G–T minus J) as a v1.1 spec revision OR by calibrating hedge geometry against the ICT-services-direct sub-component (Divisions 62–63) per the §1 sub-aggregate-substitutability flag rather than against the Section J aggregate `β_composite`. Promoting a Stage-1 PASS verdict to Stage-2 / Stage-3 dispatch *without* this disclosure is anti-fishing-banned. The R5 robustness arm "primary spec on (Sections G–T minus J)" is hereby PRE-AUTHORIZED for v1.1 spec revision under the conditional gate: user explicitly invokes the design change (option from disposition memo Option C) OR Stage-2 dispatch surfaces the need (i.e., post-Stage-1 PASS, Stage-2 M-sketch authoring requires the decomposition to calibrate hedge geometry).

- **§9.17 CORRECTIONS-κ result-memo disclosure pre-pin (NEW v1.0.2; extends §9.16).** Any Phase-3 result memo emitted from this iteration MUST include a NEW §11.X section titled "Realized-vs-anticipated data gap disclosure (CORRECTIONS-κ)" containing the following pre-pinned content blocks:
  - **(a) Verbatim citation of Task 1.1 DE-reported FLAG-A + FLAG-B**: `cell_count` realized `[94, 267]` vs Y feasibility memo §1.1 baseline `[700, 1200]` (5-7× below; 1 month at 94, 55% of months below 150); `raw_share` realized `[0.014, 0.031]` vs spec §5.1 v1.0.1 expected `[0.04, 0.10]` (1.3-3× below); logit-derivative `[33, 73]` vs spec-anticipated 2.34× across-support baseline (3-7× larger amplification at the realized range vs the v1.0.1-anticipated range).
  - **(b) Comparison of primary `β̂_composite` to R1 (2021 regime dummy) and R3 (raw-OLS) `β̂_composite` values, with explicit sign-AGREE-or-ESCALATE adjudication per §6 v1.0.2 κ-tightened condition**: report primary `β̂_composite`, R1 `β̂_composite`, R3 `β̂_composite` as a 3-row table; classify each R-row as sign-AGREE or sign-DIFFERENT relative to primary; if either R1 OR R3 is sign-DIFFERENT, the verdict re-routes from PASS to ESCALATE per §6 v1.0.2 κ-tightening regardless of §7.1 aggregate classification.
  - **(c) Acknowledgment that the 94-cell rare-month observation (2024-10-31) is post-2021 era and therefore captured by R1's regime-dummy interaction term**: this is a positive design feature of R1 under realized κ-amplification (R1 specifically catches the methodology-break × rare-event interaction in the post-2021 era), not a confound; the result memo must state this explicitly to avoid the methodologically-incorrect reading "R1 didn't address the 94-cell observation."
  - **(d) Sub-aggregate-substitutability ASR mapping**: if primary PASSes, the result memo flags whether the result was driven by the BPO-relevant CIIU Rev. 4 Divisions 62-63 sub-aggregate (Computer programming + Information service activities; the ICT-services-direct sub-component per §1 sub-aggregate-substitutability flag) vs the non-BPO Divisions 58-61 sub-aggregate (Publishing, Motion picture/video, Programming & broadcasting, Telecommunications). This sub-component decomposition is NOT in Stage-1 scope per §9.11 (Stage-1-only scope) and the Stage-1 spec does not pre-commit a Division-level disaggregation; the §11.X disclosure flags this question for Stage-2 dispatch brief inheritance, defers the empirical answer to Stage-2, and binds Stage-2 M-sketch hedge geometry to the Divisions 62-63 sub-component per §9.16 already-pinned discipline.
- **Promotion gate.** Promoting a Stage-1 PASS verdict to Stage-2 / Stage-3 dispatch *without* this §11.X disclosure section is anti-fishing-banned per §9.5 HALT-disposition discipline (the disclosure is a result-memo content requirement, not an analysis requirement; failure to include it is a documentation deficiency that blocks downstream stage promotion). The CORRECTIONS-κ disclosure inheritance is the structural-equivalent of Pair D Stage-1 MEMO.md §10 narrative-tightening pattern (project memory `project_pair_d_phase2_pass`), where Pair D's 3-way review verdicts (Code Reviewer PASS_WITH_NITS / Reality Checker ACCEPT_WITH_FLAGS / Senior Developer ACCEPT_WITH_REMEDIATION) were integrated into Pair D MEMO §1 + §6 + §10 narrative tightening rather than silently absorbed; the §11.X CORRECTIONS-κ disclosure mirrors this discipline by inheriting Task 1.1 DE FLAGs + v1.0.2 κ-tightened R-row adjudication into the Phase-3 result memo as load-bearing content rather than as a footnote.
...
| 15 | §9 Anti-fishing invariants: 17 enumerated invariants (v1.0.2; v1.0.1 had 16) covering threshold immutability, Y-construction immutability, sample-window immutability, lag-set immutability, HALT protocol, escalation-as-pre-authorization, sha256 governance, X re-use disclosures, Stage-1-only scope, a_s/a_l out-of-scope, off-chain Y mandate, free-tier mandate, code-agnostic discipline, §9.16 compositional-accounting verdict-interpretation, §9.17 NEW v1.0.2 CORRECTIONS-κ result-memo §11.X disclosure pre-pin | YES |
| 16 | Stream-watchdog mitigation: chunked authoring ≤200 lines per Edit/Write call with wc -l verification between | YES |
| 17 | Sentinel `<to-be-pinned-after-recompute>` intact at frontmatter `decision_hash` and `decision_hash_v1_0_2` fields for orchestrator post-emission sha256 patching | YES |
| 18 | §1 CORRECTIONS-κ post-Task-1.1 acknowledgment paragraph present (FLAG-A cell_count [94, 267] vs memo [700, 1200]; FLAG-B raw_share [0.014, 0.031] vs spec [0.04, 0.10]; logit-derivative [33, 73] vs 2.34× baseline = 3-7× larger amplification; anti-fishing posture explicit; Option A from disposition memo) | YES |
| 19 | §5.1 realized-vs-anticipated gap quantified (raw_share + logit-derivative numerics inline; 10-cell sampling-noise variance worked example; v1.0.2 R1 + R3 sign-AGREE-or-ESCALATE operational discipline cross-referenced) | YES |
| 20 | §6 v1.0.2 escalation tightening sub-paragraph present (primary-vs-{R1, R3} sign-inconsistency = MANDATORY ESCALATE per §3.3 Clause-A; R2 + R4 explicitly excluded from κ-tightening; §8.1 step 4(a) PASS gate layered) | YES |
| 21 | §9.17 NEW v1.0.2 invariant present binding Phase-3 result-memo §11.X disclosure section with pre-pinned content blocks (a) verbatim FLAG-A + FLAG-B citation, (b) primary-vs-R1-vs-R3 3-row sign adjudication table, (c) 94-cell rare-month R1-coverage acknowledgment, (d) Divisions 62-63 vs 58-61 sub-aggregate-substitutability ASR mapping; promotion gate to Stage-2/3 conditional on §11.X presence | YES |
| 22 | Phase-4 → Phase-3 typo fix applied at all 5 v1.0.1 occurrences (lines 40, 191, 224, 258, 374); revision_history line 18 retains "Phase-4 → Phase-3" intentionally to describe the typo fix itself; phase-numbering aligned to plan v1.1 (Phase 3 = memo + 3-way review; Phase 4 = disposition) per RC FLAG-2 closure 2026-05-05 | YES |

---

**End of spec.** All 9 sections present plus §10 self-review; no "TBD" placeholders remaining; all thresholds pinned in §3 + §6 + §7 + §9 with concrete numerics; §8 verdict tree deterministic with synthetic-tuple walk in §8.2; §3.4 + §5.5 escalation suite pre-authorized with concrete disjunctive thresholds; §6 methodology-break disposition pre-pins primary + R1, BANS restrict-to-≥2022 + v1.0.2 κ-tightening of R1+R3 sign-AGREE escalation gate; §9 anti-fishing invariants explicit (17 invariants in v1.0.2 incl. NEW §9.17 CORRECTIONS-κ result-memo §11.X disclosure pre-pin), Phase-A.0-aligned, Pair-D-aligned, and CORRECTIONS-η/θ/ι/κ-compliant. v1.0.2 Phase-4 → Phase-3 typo fix at 5 occurrences. Ready for orchestrator post-emission sha256 patching, plan emission, and 2-wave verification (RC + purpose-matched specialist closure-only on diff per `feedback_two_wave_doc_verification`).
