---
spec_version: 1.0
decision_hash: 72ebe2561ac8b724df671e7d312e604b6c9a60419ee179ebc45a2454bfa345cc
...
verifier_v1_wave1: pending (Reality Checker — Wave 1, post-emission)
verifier_v1_wave2: pending (Model QA Specialist fresh instance — Wave 2, post-emission)
revision_history:
  - v1.0 2026-05-04 initial draft (Model QA Specialist; chunked-authoring discipline per `feedback_background_agent_stream_watchdog_timeout`)
---
...
- **Lag-window loss:** `k = 12` drops the 12 leading X-panel months. Raw 2015-01 → 2026-03 spans 135 monthly cells; lag-12 loss leaves 135 − 12 = 123 candidate months pre-methodology-break treatment. The Y feasibility memo §1.1 expects realized N = 134, which assumes the 12 leading lag-loss months come from outside the 2015-01 → 2026-03 window (i.e., lag observations from 2014-01 through 2014-12 of `log(COP_USD)` are joined into the panel and consumed as regressors but the dependent variable starts at 2015-01). **This spec adopts the feasibility-memo convention:** Y dates are 2015-01 → 2026-03 inclusive (135 monthly observations less the publication-lag tail), and lag-12 X observations from 2014-01 through 2014-12 are joined as regressors. Realized N is therefore expected to be approximately **134** (one-month tolerance for end-of-window publication-lag drop).
...
- **Y_s1 — DANE EMS Section J nominal-income index** (Encuesta Mensual de Servicios; aggregate index, NOT micro-data; base 2018=100; monthly cadence; window 2018-01 → 2026-03; N=87 post-lag-12; pre-committed 2020-01 redesign dummy). Used in §7 R5-equivalent sensitivity is NOT included in the R1-R4 robustness universe of §7 (which are all single-row alternatives to the primary OLS specification). Y_s1 is reserved for downstream cross-validation if the §7 R-row consistency classification yields MIXED.
...
- **Fallback (i):** 3-month rolling-average `Y_t` (each month is the average of months `t-1`, `t`, `t+1` raw shares; logit-transform on the averaged share). NOT pre-committed as primary; reserved for a §6-clause fallback only if the primary OLS produces residual variance that exceeds 5× the median monthly variance estimated from Pair D's Section G–T precedent at the same window.
