...
- **v2 Anvil fork target (Ethereum-side)**: Ethereum mainnet, chainId 1, fork block height pinned at v2 dispatch within ±24h of the v2 Celo-side fork to avoid cross-chain oracle drift, recorded in `results/path_a_v2_fork_manifest.md`.
  ...
  - FALLBACK RPC: `https://eth.llamarpc.com` or `https://rpc.ankr.com/eth` — selection at v2 dispatch with chosen endpoint recorded in the v2 manifest. Public Ethereum RPCs support archive-depth reads but with no SLA and may rate-limit archive queries more aggressively than head-state queries; if the fallback also cannot honor the archive depth, `Stage2PathAArchiveNodeDepthInsufficientFree` fires for orchestrator adjudication.
...
**End of spec v1.2.** Authored 2026-04-30 PM under auto-mode; v1.1 revision 2026-05-02 under auto-mode applying the Wave-1 (Reality Checker) + Wave-2 (Software Architect) verification matrix from v1.0 (sha256 `56fa06b8222789eb6902227a09661728a899b464bc155036a3328d746d644665`); v1.2 revision 2026-05-02 under auto-mode applying CORRECTIONS-δ for the user budget directive change to FREE-TIER ONLY (predecessor v1.1 sha256 `de4e8cdcb1af99d96f7657f7e9b397c8caf9196e59d30d5e9afce8e72265128b`). v1.1 verifier cycle PASS-WITH-NITS (closed). v1.2 pending fresh 2-wave verification (Reality Checker + Software Architect) per `feedback_two_wave_doc_verification` before commit, scoped to the budget-pin scrub surface (frontmatter + §5 + §6 + §9 + §10.1 + change-log) since v1.1 BLOCK/FLAG closures elsewhere are not regressed. Spec sha256 to be pinned by orchestrator at commit time.
