...
status: REV-1 (awaiting three-way review)
...
Market participants observe: prior TRM levels, BanRep remittance releases (monthly with 45-day lag), contemporaneous global macro (VIX, DXY, oil), and Colombian fiscal/monetary signals. Remittance releases are low-salience relative to CPI releases — minimal market-anticipation protocol (Basco & Ojeda-Joya 2024 Borrador 1273).
...
Sample: 947 weekly observations, 2008-01-04 to 2026-04-17, identical to Rev-4 frozen panel. Decision-hash extended (not replaced) per Rev-4 panel invariants.
...
`|β̂_Rem / SE(β̂_Rem)| > 1.645` at α = 0.10 two-sided.

**Rationale for two-sided** (resolves Model-QA BLOCK-1): no defensible economic sign prior for remittance surprise on FX vol. Literature gives competing predictions:
- Stabilizer hypothesis: remittance inflows smooth household income → reduce FX pressure → negative β_Rem.
- Stress-response hypothesis: remittance surprises are typically upward (diaspora response to crisis) → coincide with depreciation episodes → positive β_Rem.
- IMF GIV template (Adrian et al. 2026): 1% inflow → +40bp parity deviation — a level effect, ambiguous for vol.

Halving the rejection region via one-sided test without an ex-ante sign prior would be indefensible. Two-sided gate preserves symmetric inference.

### 4.5 Minimum-detectable-effect-size (MDES) pre-commitment

Effective sample size N_eff ≈ 200 (HAC-autocorrelation-adjusted from nominal N=947, consistent with Rev-4 CPI exercise effective DOF).

At α = 0.10 two-sided, 80% power: MDES ≈ 2.80 / √N_eff ≈ 0.198 standard deviations of residualized `RV^{1/3}_w`.

Approximate raw units: `MDES ≈ 0.20 · SD(RV^{1/3}_w,residualized) ≈ 0.030` (using Rev-4-observed SD ≈ 0.15).

**Inconclusive-verdict rule (resolves Model-QA BLOCK-2):**
If `|β̂_Rem / SE(β̂_Rem)| < 1.645` AND `|β̂_Rem / SD(RV^{1/3}_w)| < 0.20` → verdict = **INCONCLUSIVE** (statistically null but underpowered).
If `|β̂_Rem / SE(β̂_Rem)| < 1.645` AND `|β̂_Rem / SD(RV^{1/3}_w)| ≥ 0.20` → verdict = **FAIL** (statistically null, sufficiently powered).
If `|β̂_Rem / SE(β̂_Rem)| > 1.645` → verdict = **PASS** (sign published separately).

### 4.6 Step-interpolation protocol

**LOCF (last-observation-carried-forward) anchored on BanRep release dates.** For week w with Friday-close date `d_w`, remittance surprise = AR(1) residual of the most recent monthly release with `release_date ≤ d_w`.

Rationale: LOCF is the causal interpretation — reflects the information actually available to FX participants at week w (Kuttner 2001; Gürkaynak-Sack-Swanson 2005). Forward-fill / next-week-fill would introduce look-ahead bias.

Release date = actual BanRep publication date, not reference-period month-end. When release dates are missing (pre-2015), surrogate release date = 15th of the month following the reference-period. This concession is flagged in the vintage-discipline section (§4.8).
...
**Caveat on pre-2015:** BanRep archived release-date metadata is unreliable before 2015. For the pre-2015 subsample, the primary uses reference-period-month + 45-day lag as a proxy release date; the spec acknowledges this reduces vintage-discipline strictness for ~382 of the 947 weekly observations. Post-2015 (roughly 565 weekly observations), full vintage discipline applies.
...
|---|---|---|
| T1 | Mincer-Zarnowitz exogeneity of primary RHS | AR(1) surprise should be orthogonal to controls; regress `ε^{Rem}_w` on lagged controls; joint F-stat with p > 0.10 for PASS. |
...
| T3a | Statistical significance of β̂_Rem (two-sided) | |β̂/SE| > 1.645 at α=0.10. |
...
| T5 | Normality of residuals (Jarque-Bera) | Diagnostic; robustness implication flagged but not blocking. |
...
## 6. Sensitivity sweep (pre-registered rows for §8 forest plot)

1. **A1-R (monthly-cadence)**: same surprise series at monthly frequency (n ≈ 216 obs).
2. **A4 (release-day-excluded weekly)**: exclude weeks containing BanRep remittance release day.
3. **Release-day-only weekly subsample**: converse of A4 (n ≈ 216 obs).
4. **Pre-2015 subsample** (proxy-release-date regime).
5. **Post-2015 subsample** (full vintage-strict regime).
6. **Dec-Jan excluded** (removes December and January weeks; n ≈ 789).
7. **SARIMA(1,0,0)(1,0,0)_12 surprise** (seasonal-AR residual).
8. **Current-vintage (revised) remittance series**.
9. **Quarterly corridor reconstruction (US-subset)**: from Basco & Ojeda-Joya 2024 Borrador 1273 methodology.
10. **Alternate LHS: log(RV_w)** — tests LHS-transform sensitivity.
11. **GARCH-X variance-equation placement** — surprise in σ² equation rather than μ equation.
12. **Petro-Trump Jan-2025 event-dummied** — primary + dummy around 2025-01-21 to 2025-02-02 window.
13. **Event-study co-primary (Petro-Trump)** — see §7.

**Anti-fishing protocol**: material-mover §9 spotlight in NB3 is emitted ONLY if primary T3b PASSes. Under FAIL or INCONCLUSIVE verdicts, §9 cells emit placeholders referencing the pre-registered sensitivity rows above and asserting that no post-hoc spotlight is performed.

## 7. Event-study co-primary

**Event**: Jan 26, 2025 Trump-Petro tariff-deportation standoff. Documented +100% growth in Littio USDC-account opens within 48h (per `OFFCHAIN_COP_BEHAVIOR.md` §1, confirmed by 10+ news outlets).

**Window**: [w = -1, w = +5] weekly, 7 weeks total. Event date = 2025-01-24 (Friday before the weekend announcement).

**Test statistic**: cumulative abnormal `RV^{1/3}` over window, standardized by panel-pre-event SD. One-sample t-stat tested at α = 0.10 two-sided.
...
Extend the Rev-4 frozen decision-hash (`6a5f9d1b05c18defd8b30c4b3cef6af896d6e45a2a26c1c60aa342da0a5a443c` per current nb1_panel_fingerprint.json) with the new remittance-column schema hashes:

`decision_hash_rev1 = SHA256(decision_hash_rev4 || sort(remittance_col_spec_hashes))`

Where `remittance_col_spec_hashes` includes: primary-RHS hash, A1-R hash, regime-dummy hash, event-dummy hash, release-day-indicator hash, LOCF-surrogate flag, AR(1) params hash, SARIMA params hash, Basco-reconstruction recipe hash.
...
4. **Pre-existence in corpus**: `REMITTANCE_VOLATILITY_SWAP.md` file-modification-time 2026-04-02 predates CPI-FAIL verdict (2026-04-19) by 17 days — establishing remittance as a candidate mechanism before the CPI FAIL was known.
...
| 3 | Alternate-LHS sensitivity | `log(RV_w)` as sensitivity row 10 of §6 | Bollerslev-Zhou 2002 asymptotic-distribution argument; log(RV) and RV^{1/3} are monotone-related so sign-flip is informative. | Forest-plot row 10 (alternate-LHS) appears with β̂/SE computed against log(RV_w). |
...
| 7 | AR order | Primary: AR(1). Sensitivity row: SARIMA(1,0,0)(1,0,0)_12 | Parsimony + continuity with Rev-4; seasonal sensitivity addresses documented Dec remittance spikes without multiplying primary parameters. | Both residual series computed; sensitivity row 7 in §6 emitted. |
...
| 11 | GARCH(1,1)-X parametrization | Primary: surprise in MEAN equation. Sensitivity row 11 of §6: surprise in VARIANCE equation. | Rev-4 continuity for primary (same null as OLS β_Rem); MQ-FLAG-5 vol-of-vol alternative handled via sensitivity. | Spec §4.2 states both + which is primary; NB2 implements both. |
| 12 | Dec-Jan seasonality treatment | Sensitivity row 6 of §6: Dec-Jan excluded subsample | AR(1)-residualized surprise already partially absorbs seasonality; Dec-Jan-excluded sensitivity tests whether primary β̂ survives seasonal removal. | Sensitivity row 6 emitted with n ≈ 789; comparison to primary reported in NB3. |
...
- Adrian, T. et al. (2026). "Stablecoin inflows and FX markets." IMF Working Paper 2025/141.
...
- Basco, E. and Ojeda-Joya, J.N. (2024). "Remittance cyclicality in Colombia." Banco de la República Borradores de Economía 1273.
